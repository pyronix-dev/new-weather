...
export default function WindPage({ initialUser }: { initialUser: any }) {
                ...
                {!loading && weather ? (
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-10">
                        <div className="mb-8 border-b border-slate-100 pb-6">
                            <h1 className="text-3xl sm:text-4xl font-bold text-slate-800 mb-2">Vent</h1>
                            <p className="text-xl text-slate-500 capitalize">{formatDate(weather.date)} - {cityName}</p>
                        </div>

                        <div className="flex justify-center mb-12">
                            <div className="bg-teal-50 rounded-full w-64 h-64 flex flex-col items-center justify-center border-4 border-teal-100">
                                <p className="text-teal-600 font-medium mb-2 uppercase tracking-wide text-sm">Rafales Max</p>
                                <div className="text-7xl font-bold text-slate-800">
                                    {Math.round(weather.windMax)}
                                </div>
                                <div className="text-2xl text-slate-400 font-medium">km/h</div>
                            </div>
                        </div>

                        <div className="h-96 w-full">
                            <h3 className="text-lg font-bold text-slate-800 mb-6">Vitesse du vent par heure</h3>
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={hourlyData} margin={{ top: 10, right: 0, left: -10, bottom: 0 }}>
                                    <defs>
                                        <linearGradient id="colorWindBig" x1="0" y1="0" x2="0" y2="1">
